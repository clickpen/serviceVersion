...
/*            var off1 = 0;
    ...
    return {x1:bb1.x,y1: bb1.y,x2:bb2.x,y2:bb2.y};
        },
                ...
                return {
                    linestr:"M"+fromx+","+fromy+" L"+tox+","+toy,
                        ...
                        arrowposother:{
                            x:(fromx+tox)/2,
                            y:(fromy+toy)/2,
                            transform:((fromx+tox)/2)+","+((fromy+toy)/2)+"r"+(c/Math.PI)*180+(c>Math.PI/2?("t-8,-8"):("t8,8"))
                        }
                }
            },
            ...
            label = this.text(pos.arrowpos.x, pos.arrowpos.y, edge.label||"")
                .attr({fill: "#000", "font-size": "8px"})
                .transform(pos.arrowpos.transform)
                .attr(edge.arrowStyle||{}),
            labeldown=this.text(pos.arrowposother.x, pos.arrowposother.y, edge.labeldown||"")
            .attr({fill: "#000", "font-size": "8px"})
            .transform(pos.arrowposother.transform)
            .attr(edge.arrowStyle||{});
        return {
            redraw:function(){
                ...
                labeldown.attr(pos.arrowposother).attr(edge.arrowStyle||{}).transform(pos.arrowposother.transform);
                labeldown.attr({x:pos.arrowposother.x,y:pos.arrowposother.y});
            },
                //高亮箭头时会调用该方法
                highlight:function(){
                    ...
                    labeldown.data("_normal",{
                        stroke:label.attr("stroke"),
                        fill: label.attr("fill")
                    });
                    labeldown.attr({"stroke":"#FF9900","fill":"#FF9900"});
                },
                //清除高亮
                unhighlight:function(){
                    triangle.attr(triangle.data("_normal"));
                    label.attr(label.data("_normal"));
                    labeldown.attr(label.data("_normal"));
    ...
    (!Raphael.fn.rgraphnode ) && (Raphael.fn.rgraphnode=function(node){
        ...
        getSize  = function(text){
            ...
            var s = document.createElement("span"),size = {};
            s.id="_rgraph_span";
 ...
 // node._shape = paper.rect(node.point[0]-size.w/2,node.point[1]-size.h/2, size.w, size.h,3)
    //node._shape = paper.circle(node.point[0],node.point[1], size.r)

   node._shape = paper.image(node.rectStyle.path,node.point[0],node.point[1], size.w,size.h)
       .attr({stroke: "#ccc",fill:"#ccc","stroke-width":1,"cursor":"pointer"})
       .attr(node.rectStyle||{});
   node._text = paper.text(node.point[0]+15,node.point[1]-15,text).attr({"cursor":"pointer","font-size":12}).attr(node.textStyle||{});
    return {
            //移动节点时会调用该方法
            move:function(dx,dy){
    ...
    RGraph.prototype={
        ...
        _bind:function(){
            ...
            if(node && node.data("nodeId")){//拖动节点
                ...
                (me.onnodestartdrag||function(){}).call(me,evt,draggingnode);
            }        
            });
            ...
            if(me.canvasdragging && now-me.canvasdragging.timestamp >50 ){
            ...
            if(me.nodedragging && now-me.nodedragging.timestamp >50){
            ...
            //console.log(evt.clientX+","+evt.clientY+" "+dx+","+dy);
                ...
                (me.onnodedragging||function(){}).call(me,evt,node);
            }
            //鼠标放在节点上，高亮节点和相关的箭头
            //排除现有行为
            ...
            if(hovernode && hovernode.data("nodeId")){
                ...
                if(me._lastHoverNode!=me.hovernode){//进入node
                    if(me._lastHoverNode ){ //触发之前节点的unhighlight
                        ...
                        (me.onnodemouseout||function(){}).call(me,evt,me._lastHoverNode);
                    }
                    ...
                    (me.onnodemouseover||function(){}).call(me,evt,me.hovernode);
                }
            ...
            (me.onnodeenddrag||function(){}).call(me,evt,node);
            me.nodedragging = false;
        ...
        layout:function(){
            ...
            //draw
            var radius = 50,
                width = Math.max(me.nodes.length*radius*1,me.width*0.4),
                height = Math.max(me.nodes.length*radius*0.8,me.height*0.4),
                g = me,
        ...
        addNode:function(node){
            ...
            node.point=[vb.x+Math.random()*vb.w*0.7+vb.w*0.15,vb.y+Math.random()*vb.h*0.7+vb.h*0.15];
            me.nodes.push(node);
            me._drawNode(node);
            return true;
        },
        ...
        center:function(nodeId){
            ...
            vb.x = (node.point[0]||0)-me.width/2;
            vb.y = (node.point[1]||0)-me.height/2;
        ...
        highlight:function(node){
            ...
            node._shape.attr({"stroke":"#FD6D03","fill":"#FD6D03"});
            node._text.attr({ "stroke":"#FD6D03","fill":"#FD6D03)"});
        }
    };
}(window)
